...
## Documentos de marca (aba "Marcas") — 2026-09-26
Kelvara, NeuroStat, DRE e Ødemark A.: 12 documentos, 24 lados (frente+verso pra duplex; terminal DRE e mensagens NeuroStat são 1 folha). Visual = mockups aprovados pelo Max, com texto 100% editável.
- **Runtime:** `tools/brand.js` (carrega DEPOIS de `editor.js`). `BrandText extends fabric.Textbox` (desenha com as métricas do navegador; quebras de linha originais em `brandWraps` valem só enquanto texto/fonte/largura não mudam; markup `**negrito**`, `[[censura]]`, `\` escapa), `BrandArt` (arte travada; NUNCA serializa o dataURL no histórico), troca frente/verso com histórico separado por lado, "Baixar frente e verso" via StaticCanvas. Packs em `tools/vendor/brand/<doc>_<lado>.js` (lazy, base64) + `brand-index.js`.
- **Fonte dos packs:** `tools/brand-build/` — `gen_*.js` geram os mockups HTML (marcadores `data-e`/`data-hand`/`data-r`/`data-obj`/`data-slot`), `build_individual.js` extrai cada raiz, `brand_build.js` (CDP no Edge, roda com `node brand_build.js [doc ...]`) extrai campos+objetos e renderiza a arte com o texto escondido, `fidelity.js` faz diff pixel-a-pixel editor vs mockup. Ordem: `node gen_kelvara.js && node gen_neurostat.js && node gen_dre.js && node gen_odemark.js && node build_individual.js && node brand_build.js`. Pra mudar um documento: editar o `gen_*.js`, regerar e rebuildar só aquele doc.
- **Técnica que importa:** a extração usa CSS `zoom` = 2×k (não `deviceScaleFactor` emulado — este mantém o layout a 1x e arredonda ascent/linha). Hospedagem do teste: o Edge headless deixa perfil temporário GIGANTE em disco (já encheu o C: uma vez) — os scripts apagam o perfil no fim (`taskkill /T`), conferir se sobrou `edge_profile_*`.
- **Regras dos marcadores:** `data-e` não pode ser `display:inline` nem conter span com cor/fonte/tamanho diferente (só `<b>`, `<br>`, `[data-r]`); o extrator avisa. `data-obj` vira imagem solta com alfa (carimbo/ring/logo/selo); a arte é renderizada SEM eles e SEM o texto.
- **Não fazer:** reintroduzir textura fotográfica em Kelvara/NeuroStat (só existem desde 2011, 100% digitais).
